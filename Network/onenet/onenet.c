...
#define PROID "sKVsYW0915"

#define ACCESS_KEY "xeVbwXUJmVX1VS/vBVfVCXMVxa2H+ZNF4QF3P80/OwU="
...
unsigned char OneNet_FillBuf(char *buf)
{
    ...
    memset(text, 0, sizeof(text));
    sprintf(text, "\"Temperature\":[{\"v\":%d}],", temperature);
    strcat(buf, text);
    ...
    memset(text, 0, sizeof(text));
    sprintf(text, "\"CH4\":[{\"v\":%.2f}],", CH4); // 添加CH4浓度
    strcat(buf, text);

    memset(text, 0, sizeof(text));
    sprintf(text, "\"Lon\":[{\"v\":%.6lf}],", f_longitude);
    strcat(buf, text);

    memset(text, 0, sizeof(text));
    sprintf(text, "\"Lat\":[{\"v\":%.6lf}],", f_latitude);
    strcat(buf, text);
